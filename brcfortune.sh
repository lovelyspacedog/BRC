...
readonly __BRCFORTUNE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$__BRCFORTUNE_DIR/_DEPENDENCY_CHECK.sh"

brcfortune() {
    if ! ensure_commands_present --caller "brcfortune" fortune tput head; then
        return 123
    fi
    ...
    if [[ "$custom_flag" == true ]]; then
        ...
        if [[ "$custom_title" == "-" ]] || [[ "$custom_fortune" == "-" ]]; then
            ...
            local expanded_cmd="brcfortune --custom $expanded_title $expanded_fortune"
