...
3. The script will:
   ...
   - Preserve `_PREAMBLE.sh` (never overwritten, only generated if missing)

4. After updating, check the backup directory for any custom files:
   ```bash
   ls ~/BASHRC.backup.*
   ```
   Port over any custom scripts you had in your old installation. Note that `_PREAMBLE.sh` is preserved automatically during updates.
...
2. **Modify plugins**: Edit individual plugin files in `~/BASHRC/` (note: these will be overwritten on updates)
3. **Add aliases**: Edit `~/BASHRC/_ALIASES.sh` (note: this will be overwritten on updates)
...
**Note**: `_PREAMBLE.sh` is not tracked in the repository. It is generated automatically by `___INSTALL.sh` during installation if it doesn't exist. This file is user-customizable and will never be overwritten by updates.
